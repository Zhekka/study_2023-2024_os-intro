...
Для этого введем `gedit prog1.sh`, чтобы вызвать gedit редактор и создать файл для скрипта. После ввожу скрипт необходимый заданию (рис. [-@fig:001]): 
...
Сделаем файл исполняемым с помощью `chmod +x prog1.sh` и проверяем, сработал ли скрипт. На скриншоте можно увидеть, что все работает (рис. [-@fig:002]).

![Результат работы скрипта №1](image/2.png ){#fig:002 width=100%}

2. Напишем пример командного файла, обрабатывающего любое произвольное число аргументов командной строки. (рис. [-@fig:003]):
...
3. Напишем командный файл — аналог команды ls (без использования самой этой команды и команды dir), который выдавал информацию о нужном каталоге и выводил информацию о возможностях доступа к файлам этого каталога (рис. [-@fig:005]):
